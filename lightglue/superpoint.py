...
class SuperPoint(Extractor):
    ...
    def __init__(self, **conf):
        ...
        url = "https://github.com/hihosilvers/LightGlue/releases/download/v0.1_arxiv/superpoint_mit.pth"
    ...
    def forward(self, data: dict) -> dict:
        ...
        for i in range(b):
            ...
            descriptors_list.append(d.squeeze(0).transpose(0, 1))

        # Return stacked tensors as expected by Extractor
        if len(keypoints) == 1:
            # Handle single batch case
            return {
                "keypoints": keypoints[0][None],  # Add batch dimension
                "keypoint_scores": scores_list[0][None],  # Add batch dimension
                "descriptors": descriptors_list[0][None],  # Add batch dimension
            }
        else:
            # Stack multiple batches
            return {
                "keypoints": torch.stack([kp for kp in keypoints], dim=0),
                "keypoint_scores": torch.stack([s for s in scores_list], dim=0),
                "descriptors": torch.stack([d for d in descriptors_list], dim=0),
            }
